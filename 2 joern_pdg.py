...
def parse_options():
    parser = argparse.ArgumentParser(description='Extracting PDGs step1.')
    parser.add_argument('-i', '--input', help='The dir path of input', type=str, default='./data/source')
    parser.add_argument('-o', '--output', help='The dir path of output', type=str, default='./data/bins')
...
def joern_parse(file, outdir):
    ...
    os.system('joern-parse $file --language c -o $out')
...
def joern_export(bin, outdir, repr):
    ...
    if repr == 'pdg':
        os.system('joern-export $bin'+ " --repr " + "pdg" + ' -o $out')
        try:
